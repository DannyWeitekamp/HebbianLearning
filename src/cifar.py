...
def plot_images(images,show=True,subplots=(15,15)):
    ...
    for k,im in enumerate(images):
        ...
        if(len(data.shape) == 2):
            ...
            data = data / max(np.max(np.abs(data)),1e-7)
            data = data / 2.0 + 0.5
...
net = HebbianLayer(3,max_active_post_neurons=1,
                    add_at_sum=.6,
                    add_max_corr = .7,
...
run2,run3 = False,False

images_seen = 0 
for i, data in enumerate(trainloader, 0):
    inputs, labels = data
    # plot_images(inputs[0].unsqueeze(0),show=True)
    ...
    #     sub = net.conv.weight[:,:3*49].view(-1,3,7,7)
    #     sub.contiguous()
    #     # print(torch.max(a))
    #     imshow(inputs[0],show=False)
...
if(run3):
    ...
    model = MLPNet(last_net.num_neurons,5,5)
else:
    model = MLPNet(last_net.num_neurons,13,13)  
# 
# model = MLPNet(last_net.num_neurons,4,4)
